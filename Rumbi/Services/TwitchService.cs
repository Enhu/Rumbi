...
namespace Rumbi.Services
{
    public class TwitchService
    {
        ...
        public async Task<string> Authenticate()
        {
            using (var httpClient = new HttpClient())
            {
                using (var request = new HttpRequestMessage(new HttpMethod("POST"), "https://id.twitch.tv/oauth2/token"))
                {
                    ...
                    TwitchTokenResponse json = JsonConvert.DeserializeObject<TwitchTokenResponse>(responseBody);
        ...
        public async Task<string> GetStreamGame(string accesssToken, string channel)
        {
            using (var httpClient = new HttpClient())
            {
                using (var request = new HttpRequestMessage(new HttpMethod("GET"), $"https://api.twitch.tv/helix/streams?user_login={channel}"))
                {
                    request.Headers.TryAddWithoutValidation("Authorization", $"Bearer {accesssToken}");
                    request.Headers.TryAddWithoutValidation("Client-Id", $"{RumbiConfig.Config.TwitchClientId}");
